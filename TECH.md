...
| `/api/auth/logout` | POST | 清除登录令牌（HttpOnly Cookie 失效），用于"不同意承诺即返回验证入口" |
...
| 路由 | 方法 | 说明 |
|------|------|------|
| `/api/favorite/toggle` | POST | 收藏 / 取消收藏（需登录） |
| `/api/favorite/list` | GET | 当前用户收藏列表（跨设备同步） |
| `/api/favorite/status` | POST | 批量查询收藏状态（单次最多 100 pid） |
...
| 单 IP 留言（未登录） | 3 次/小时 + 8 次/天 |
| 单 IP 留言（已登录） | 3 次/小时 + 8 次/天（与未登录共享同一限流池，防登录用户轰炸站长邮箱） |
| 全局留言 | 50 次/小时（放宽，避免大批正常用户同时留言被误限） |
| 邮箱验证 | 60 秒重发间隔 + 10 次/天/IP |
...
- 使用 QQ 邮箱 SMTP 发送验证码 + 转发留言 + 关键词订阅推送
...
| 任务 | 周期 | 说明 |
|------|------|------|
| Token 过期告警 | 启动 +30s，之后每 6 小时 | `PKU_TOKEN` 剩余 ≤5 天时给站长发告警邮件（同一过期周期仅发一次） |
| 关键词订阅扫描 | 启动 +60s，之后每 2 分钟 | 扫描水位线之后的新帖，正文包含关键词即命中，按用户合并成摘要邮件（`subscription_sent` 表去重防重复推送） |

订阅推送首次启动时水位线设为当前最大 pid，不补推历史帖；每用户每周期合并 1 封摘要（最多列 20 条），邮件含 HMAC 签名的一键退订全部链接。仅校园邮箱用户可订阅（邀请码为一次性凭证不具备订阅资格），收信邮箱强制使用注册邮箱（防借订阅向他人邮箱轰炸）。
...
| `verify_codes` | 邮箱验证码（带过期、尝试次数、IP 用于每 IP 每日限额） |
| `post_views` | 帖子浏览记录 |
| `visit_logs` | 访问日志 |
| `invite_codes` | 邀请码（码 / 备注 / 使用状态 / 使用者） |
| `report_logs` | AI 报告调用日志（IP / 服务商 / 模式 / 成败） |
| `favorites` | 帖子收藏（user_email + pid 唯一约束，跨设备同步） |
| `subscriptions` | 关键词订阅（user_email / notify_email / keyword，每用户每关键词唯一） |
...
任何报错 / 超频访问 / 受到攻击 / 服务失败 / 可疑用户事件发生时，后端 `alertAdmin()` 会：

1. **入库** `alert_logs` 表（每次都记，保证日志完整）
2. **邮件通知站长**（同 type 事件 10 分钟内只发一封，防告警邮件本身被刷爆；入库不受节流影响）
3. **数据后台展示**（admin.html 新增"安全告警"栏：趋势图 + 按类型/级别统计 + 最近 100 条记录）

| 告警类型 | 触发场景 | 级别 |
|----------|----------|------|
| `rate_limit` | AI 报告接口 / 留言接口触发频率限制（疑似轰炸） | warn |
| `ssrf` | LLM 自定义 URL 被 `assertSafeUrl()` 拦截（SSRF 攻击尝试） | error |
| `brute_force` | 验证码尝试次数耗尽 / 邀请码登录失败（暴力破解） | warn |
| `server_error` | 路由处理抛出 500 级错误（服务失败） | error |
| `token_warn` | 树洞 Token 剩余 ≤5 天 | info |

`alert_logs` 表字段：id / level / type / subject / detail / ip / notified（是否已发邮件）/ created_at。
